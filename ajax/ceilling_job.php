<?php
	...
	if($action == "Save") {
		...
		$Ceil->CeilID = $CeilID;
		$Ceil->JobID = $JobID;
		$Ceil->ProjectID = $ProjectID;
		$Ceil->JobName = $JobName;
		$Ceil->widthEstimate = $widthEstimate;
		$Ceil->longEstimate = $longEstimate;
		$Ceil->StartPointX = $StartPointX;
		$Ceil->StartPointY = $StartPointY;
		$Ceil->MaterialID = $MaterialID;
		$Ceil->ObjectWall = $ObjectWall;
		$Ceil->ReservePercent = $ReservePercent;
		$Ceil->ReserveValue = $ReserveValue;
		$Ceil->CreatedBy = $Account->Username;
